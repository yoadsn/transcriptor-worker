...
from transcriptor_worker.models import DescJson, DocFile, Submission
...
def _write_submission(tmp_path: Path, sub_id: str, files: dict[str, bytes], desc_files: list[dict]) -> None:
...
class TestPDFExtraction:
    def test_two_page_pdf_yields_two_records(self, tmp_path, src, tgt, temp_dir):
        ...
        _write_submission(tmp_path, "sub1", {"doc.pdf": pdf_bytes}, [
            {"stored_filename": "doc.pdf", "file_extension": ".pdf", "mime_type": "application/pdf"}
        ])
        submission = Submission(id="sub1", source_path="sub1")
        records = extract_pages(submission, src, tgt, temp_dir)
    ...
    def test_null_mime_type_pdf_detected_by_extension(self, tmp_path, src, tgt, temp_dir):
...
class TestImagePassthrough:
    def test_single_jpeg_yields_one_record(self, tmp_path, src, tgt, temp_dir):
        ...
        _write_submission(tmp_path, "sub2", {"photo.jpg": jpg_bytes}, [
...
class TestErrorHandling:
    def test_missing_desc_json_returns_failed_record(self, tmp_path, src, tgt, temp_dir):
        ...
        assert "desc.json" in records[0].error.lower() or "Cannot read" in records[0].error

    def test_corrupted_pdf_yields_failed_record(self, tmp_path, src, tgt, temp_dir):
        _write_submission(tmp_path, "sub-corrupt", {"bad.pdf": b"not a pdf at all"}, [
            {"stored_filename": "bad.pdf", "file_extension": ".pdf", "mime_type": None}
        ])
    ...
    def test_missing_doc_file_yields_failed_record(self, tmp_path, src, tgt, temp_dir):
        ...
        sub_dir.mkdir(parents=True)
        (sub_dir / "desc.json").write_text(_make_desc_json([
            {"stored_filename": "ghost.pdf", "file_extension": ".pdf", "mime_type": None}
        ]))
    ...
    def test_failed_doc_does_not_abort_others(self, tmp_path, src, tgt, temp_dir):
        ...
        (sub_dir / "desc.json").write_text(_make_desc_json([
            {"stored_filename": "ghost.pdf", "file_extension": ".pdf", "mime_type": None},
            {"stored_filename": "good.jpg", "file_extension": ".jpg", "mime_type": "image/jpeg"},
        ]))
